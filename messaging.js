 ...
 */
async function addMessage(conversationId, senderId, senderData, messageText) {
  // 1. Voeg bericht toe aan messages subcollection
  const messageData = {
    senderId: senderId,
    senderName: `${senderData.firstName} ${senderData.lastName}`,
    senderRole: senderData.role,
    senderProfilePic: senderData.profilePicUrl || '', // ⭐ NEW: Add profile picture
    text: messageText,
    createdAt: serverTimestamp(),
    read: false
  };
...
async function displayConversations(conversations) {
  ...
  // ⭐ NEW: Fetch profile pictures for all participants
...
/**
 * Update de unread badge in de navigatie
 */
 ...
 */
// ⭐ REPLACE the displayMessages function in messaging.js (around line 516)
